...
class Align(torch.nn.Module):
    def __init__(self, num_labels=6):
        ...
        self.config = AutoConfig.from_pretrained("kakaobrain/align-base", num_labels=6)
    ...
    def forward(self, pixel_values, attention_mask,input_ids):
        ...
        catFeatures = text_embeds  # torch.cat((self.activation(image_embeds), self.activation(text_embeds)), dim=1)
...
def TestBMMTC_Align():
    model = Align()
    model = torch.load("./mmbtc-6-model/best_model_ALIGN_Focal.pt")
    model.to(device)
    model.eval()
    datasets = ImageTextDataset("test-mmtc-6.csv")
...
def TrainBMMTC_Align():
    ...
    datasets = ImageTextDataset("train-mmtc-6.csv")
    dataloader = DataLoader(datasets, batch_size=10, shuffle=True)
    epochs = 30
    best_model = None
    ...
    for epoch in range(epochs):
        ...
        if epoch_loss < global_loss:
            ...
            best_model = model
            torch.save(best_model, "./mmbtc-6-model/best_model_ALIGN_Focal.pt")
