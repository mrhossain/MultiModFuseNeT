...
class BART_ViT(torch.nn.Module):
    ...
    def forward(self, images, input_ids, attention_mask):
        ...
        fused = self.mfb_fusion(text_output, image_output)
        #output = torch.cat((text_output, image_output), dim=1)
        output = self.linear(fused)
...
def TestTextModel():
    ...
    temp_model = BART_ViT()
    temp_model.load_state_dict(torch.load('./Model/BART_ViT_Fusion.pth'))
...
def TrainTextModel():
    ...
    for epoch in range(epoch):
        ...
        if acc > globalACC:
            globalACC = acc
            torch.save(model.state_dict(), './Model/BART_ViT_Fusion.pth')
