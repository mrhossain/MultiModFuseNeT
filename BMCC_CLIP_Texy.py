...
def TestBMMTC_CLIP(test_model):
    model = test_model #torch.load("./mmbtc-6-model/best_model_CLIP_text.pt")
    model.to(device)
    model.eval()
    datasets = CustomCLIPDataloder("test-mmtc-6.csv")
...
def TrainBMMTC_CLIP():
    ...
    datasets = CustomCLIPDataloder("train-mmtc-6.csv")
    dataloader = DataLoader(datasets, batch_size=32, shuffle=True)
    epochs = 30
    best_model = None
    ...
    for epoch in range(epochs):
        ...
        acc = TestBMMTC_CLIP(model)
        if bestACC < acc:
            bestACC = acc
            best_model = model
            torch.save(best_model, "./mmbtc-6-model/best_model_CLIP_image_Focal.pt")
