...
from transformers import ViTModel, BertModel, BertTokenizer, get_linear_schedule_with_warmup,CLIPModel, CLIPProcessor, AutoConfig, AdamW
...
class CustomTextClassification(torch.nn.Module):
    def __init__(self, num_labels=6):
        ...
        self.model = BertModel.from_pretrained("google/muril-base-cased", return_dict=True)
...
class CustoMultiModalImgaeTextClassification(torch.nn.Module):
    ...
    def forward(self, pixel_values, input_ids, attention_mask):
        ...
        mfb_fusion = self.mfb_fusion(text_embeds, image_embeds)
        #output = torch.cat((image_embeds, text_embeds), dim=1)
        output = self.linear(mfb_fusion)
        output = self.softmax(output)
        return output
...
def TestMMTC(test_model):
    #best_model_path = './Model/BMMTC_Focal_loss_CLIP_Image_BanglaBERT1e5.pt'
    dataset = CustomDataLoaderMMTC("./BMMTC6-Final/BMMTC6-Test/test.csv")
    dataloader = DataLoader(dataset=dataset, batch_size=16, shuffle=True)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print(device)
    model = test_model #torch.load(best_model_path).to(device)
    model.eval()
    predict_labels = []
    actual_labels = []
    Image_path_list = []
...
def TrainMMTC():
    ...
    for epoch in range(epochs):
        ...
        if acc > globAlaccuracy:
            globAlaccuracy = acc
            best_model = model
            torch.save(best_model, "./Model/BMMTC_Focal_loss_CLIP_Image_MuRIL1e6_MFB_Cross_Entropy.pt")
