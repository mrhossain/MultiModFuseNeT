...
class CustomTextClassification(torch.nn.Module):
    def __init__(self, num_labels=6):
        ...
        self.convert = torch.nn.Linear(768, 512)
        self.activation = torch.nn.ReLU()
        self.dropout = torch.nn.Dropout(0.1)
        self.softmax = torch.nn.Softmax(dim=1)
...
class CustoMultiModalImgaeTextClassification(torch.nn.Module):
    ...
    def forward(self, pixel_values, input_ids, attention_mask):
        ...
        #avg_func = (image_embeds + text_embeds)/2
...
def TestMMTC():
    best_model_path = './Model/BMMTC_Focal_loss_CLIP_Image_BanglaBET_Cat.pt'
...
def TrainMMTC():
    ...
    for epoch in range(epochs):
        ...
        if acc > globAlaccuracy:
            globAlaccuracy = acc
            best_model = model
            torch.save(best_model, "./Model/BMMTC_Focal_loss_CLIP_Image_BanglaBET_Cat.pt")
