...
class EmbeddingQualityMeasure:
    def __init__(self, T: int = 100, alpha: float = 91.92, r: float = 0.79446):
        self.T = T
        self.alpha = alpha
        self.r = r

    def get_quality_score(self, embeddings):
        """
        Calculates a quality score for given embeddings using T passes.

        Parameters
        ----------
        embeddings : torch.Tensor, shape (batch_size, embed_dim)
            The embeddings (either image or text).

        Returns
        -------
        quality_score : float
            The quality score for the embeddings.
        """
       ...
       # print(f"embeddings shape: {embeddings.shape}")
        ...
        # Repeat the embeddings T times
        repeated_embeds = embeddings.repeat(self.T, 1)

        #Applied dropout to the repeated embeddings
        repeated_embeds = F.dropout(repeated_embeds, p=0.5, training=True)

        #print(f"repeated_embeds shape: {repeated_embeds.shape}")

        # Normalize the embeddings
        norm_embeds = F.normalize(repeated_embeds, p=2, dim=1).cpu().detach().numpy()

        # Calculate Euclidean distances between pairs of normalized embeddings
        eucl_dist = euclidean_distances(norm_embeds, norm_embeds)[np.triu_indices(self.T, k=1)]

        # Calculate the quality score for embeddings
...
class CustoMultiModalImgaeTextClassification(torch.nn.Module):
    def __init__(self, num_labels=6, image_model=None, text_model=None):
        ...
        self.embeeds_quality = EmbeddingQualityMeasure()
        self.activation = torch.nn.ReLU()
