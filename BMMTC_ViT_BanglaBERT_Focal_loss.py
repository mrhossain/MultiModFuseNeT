...
class CustoMultiModalImgaeTextClassification(torch.nn.Module):
    ...
    def forward(self, pixel_values, input_ids, attention_mask):
        ...
        cross_aligned_features = self.cross_attention(image_output, text_output)
        output = torch.cat((output, cross_aligned_features), dim=1)
...
def TestMMTC():
    ...
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print(device)
    model = torch.load('./mmbtc-6-model/BMMTC_Focal_loss_Text_Only_lr2e-5.pt').to(device)
    model.eval()
    predict_labels = []
    actual_labels = []
    Image_path_list = []
...
def TrainMMTC():
    ...
    for epoch in range(epochs):
        ...
        if acc > globAlaccuracy:
            globAlaccuracy = acc
            best_model = model
            torch.save(best_model, "./mmbtc-6-model/BMMTC_Focal_loss_Text_Only_lr2e-5.pt")
